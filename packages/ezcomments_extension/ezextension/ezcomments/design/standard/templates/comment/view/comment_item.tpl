...
<div class="ezcom-view-comment">
            <div class="attribute-byline ezcom-comment-top">
                {if $comment.name}
                    Op {$comment.created|datetime( 'longdate' )} {'wrote:'|i18n('ezcomments/comment/view')} {if $comment.url|eq( '' )}{$comment.name|wash}{else}<a href="{$comment.url|wash}">{$comment.name|wash}</a>{/if}
                {/if}
            </div>
            {if $comment.title}
